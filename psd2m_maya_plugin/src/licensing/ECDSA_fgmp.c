//---------- ----------
// Original library from https://stuff.mit.edu/afs/sipb/project/stk/src/STk-2.2.0/Mp/fgmp-1.0b5/
// Modified by Michaelson Britt, May 2019
//---------- ----------
//
//    WELCOME TO FGMP.
//
...
#define hd(x,i)  ((((long)i)>=((long)(2*(x->sz))))? 0:(((i)%2) ? HIGH((x)->p[(i)/2]) \
    : LOW((x)->p[(i)/2])))
#define dg(x,i) ((((long)i) < ((long)x->sz)) ? (x->p)[i] : 0)
...
MP_INT *x; mp_size size;
{
    ...
    printf("realloc %08lx to size = %ld ", (long)(x->p),(long)(size));
        ...
        if (x->p && (x->p!=x->p_prealloc)) 
            x->p=(mp_limb *)realloc(x->p,size * sizeof(mp_limb));
        else if( size>sz_prealloc )
...
static int ucmp(y,x)
const MP_INT *y;const MP_INT *x;
{
    int i;
    for (i=((long)imax(x->sz,y->sz))-1;i>=0;i--) {
        if (dg(y,i) < dg(x,i))
            return (-1);
...
/* w = u * v */
void mpz_mul(ww,u,v)
MP_INT *ww;const MP_INT *u;const MP_INT *v; {
    mp_size i,j;
    mp_limb t0,t1,t2,t3;
    mp_limb cc;
    ...
    if( (ww==u) || (ww==v) )
        printf("ERROR: mpz_mul() input and output param same \n");

    _mpz_realloc(ww,(mp_size)(u->sz + v->sz));
    ...
    for (j=0; j < 2*u->sz; j++) {
        ...
        t3 = hd(u,j);
            for (i=0; i < 2*v->sz; i++) {
                ...
                cc+=t1;
                if ((i+j)%2)
                    ww->p[(i+j)/2] = LOW(ww->p[(i+j)/2]) |
                        (t2 << HALFDIGITBITS);
                    //w->p[(i+j)/2] = LOW(w->p[(i+j)/2]) |
                    //    (t2 << HALFDIGITBITS);
                else
                    ww->p[(i+j)/2] = (HIGH(ww->p[(i+j)/2]) << HALFDIGITBITS) |
                        t2;
...
MP_INT *c1;MP_INT *a;int n;
{
    ...
    if (n == 0)
        ...
        _mpz_realloc(c1,(mp_size)(a->sz + 1));
        for (i=0;i < a->sz; i++) {
            c1->p[i] = ((a->p[i] << n) | cc) & LMAX;
            cc = (a->p[i] & rm) >> (DIGITBITS -n);
            //c.p[i] = ((a->p[i] << n) | cc) & LMAX;
            //cc = (a->p[i] & rm) >> (DIGITBITS -n);
        }
...
MP_INT *z; MP_INT *x; unsigned long e;
{
    ...
    if (e==0)
        ...
        unsigned long i;
        MP_INT y;
        unsigned long digs = (e / DIGITBITS);
        unsigned long bs = (e % (DIGITBITS));
...
MP_INT *qq; MP_INT *rr; const MP_INT *xx; const MP_INT *yy;
{
    MP_INT qq_local, rr_local;
    MP_INT x_local, y_local;
    MP_INT *qq_param = qq, *rr_param = rr;
    MP_INT *x = &x_local, *y = &y_local;
    //MP_INT q, r;
    //MP_INT *q, *x, *y, *r;
    int ns,f,ccc, i, j, xd, yd;
    ...
    // Optimize result size
    for( int i=(qq->sz)-1; i>0; i-- )
    {
        if( qq->p[i]==0 )
            qq->sz--;
        else break;
    }
    for( int i=(rr->sz)-1; i>0; i-- )
    {
        if( rr->p[i]==0 )
            rr->sz--;
        else break;
    ...
    //free(q); free(r); free(x); free(y);
}

void mpz_add(zz,x,y)
MP_INT *zz;const MP_INT *x;const MP_INT *y;
{
    int mg;
    ...
    //mpz_init(z);
    if( (zz==x) || (zz==y) )
        printf("ERROR: mpz_add() input and output param same \n");
...
void mpz_sub(z,x,y)
MP_INT *z;const MP_INT *x;const MP_INT *y;
{
    MP_INT u;
    mpz_init(&u);
    mpz_set(&u,y);
...
void mpz_div(q,x,y)
MP_INT *q; const MP_INT *x; const MP_INT *y;
{
    MP_INT r; 
...
void mpz_mod(r,x,y)
MP_INT *r; const MP_INT *x; const MP_INT *y;
{
    MP_INT q;
    short sn = x->sn;
    mpz_init(&q);
...
char *s;  int base; const MP_INT *x; {
    MP_INT xx,q,r,bb;
    char *p,*t,*ps;
...
MP_INT *x; const char *s; int base;
{
    int l,i;
    int retval = 0;
    MP_INT t,m,bb;
    ...
    mpz_set_ui(&bb,(unsigned long)base);
    l=(int)strlen(s); // TODO: conversion from size_t to int, fix this
    for (i = l-1; i>=0; i--) {
        if (s[i]==' ' || s[i]=='\t' || s[i]=='\n') 
            continue;
        if (s[i] >= '0' && s[i] <= '9')
