...
const util::StringTable::StringTableItem stringTableItems_french[] =
{
	...
	{ IDS_TEXTURE_ATLAS_INFO_PADDING_FORMAT, "<font color='#f48c42'>Un remplissage de %i pixels sera appliqué</font>" },
	...
	{ IDS_LAYER_LIST_MODE_SELECTED, "Mode sélectionnés : " },
	...
	{ IDS_FBX_SAVE_PATH_DIALOG, "Exporter" }, // == TODO: TRANSLATE THIS ==
	...
	{ IDS_HELP_URL, "https://edfilms.net/fr/psd-to-fbx-documentation-et-tutoriels/" },
	...
	{ IDS_PSD_IMPORT_LABEL, "Sélectionnez un fichier PSD:" },
	...
	{ IDS_BILLBOARD_ALGO_LABEL, "Algorithme panneau d'affichage" },
	...
	{ IDS_TEXTURE_ATLAS_GROUP_LABEL, "Groupe d'atlas de textures:" },
	...
	{ IDS_TEXTURE_ATLAS_ALGO_LABEL, "Algorithme d'empaquetage:" },
	  { IDS_TEXTURE_ATLAS_ALGO_DROPDOWN_BEST_SHORT_SIDE_FIT, "Meilleur ajustement côté court" },
	  { IDS_TEXTURE_ATLAS_ALGO_DROPDOWN_BEST_LONG_SIDE_FIT, "Meilleur ajustement côté long" },
	  { IDS_TEXTURE_ATLAS_ALGO_DROPDOWN_BEST_AREA_FIT, "Meilleur ajustement superficie" },
	  { IDS_TEXTURE_ATLAS_ALGO_DROPDOWN_BOTTOM_LEFT_RULE, "Règle en bas à gauche" },
	  { IDS_TEXTURE_ATLAS_ALGO_DROPDOWN_CONTACT_POINT_RULE, "Règle du point de contact" },
	{ IDS_TEXTURE_ATLAS_OVERRIDE_LABEL, "Taille du groupe atlas :" },
	{ IDS_TEXTURE_ATLAS_OVERRIDE_CHK, "Remplacer la résolution par défaut" },
	{ IDS_TEXTURE_ATLAS_PADDING_LABEL, "Remplissage" },
	{ IDS_TEXTURE_OUTPUT_SIZE_LABEL, "Taille de sortie des textures:" },
	...
	{ IDS_WRITE_MODE_LABEL, "Format de sortie :" },
	{ IDS_WRITE_MODE_BINARY, "Binaire" },
	{ IDS_WRITE_MODE_ASCII, "ASCII" },
	{ IDS_WRITE_LAYOUT_LABEL, "== TODO: TRANSLATE THIS ==" },
	{ IDS_WRITE_LAYOUT_SINGLE, "== TODO: TRANSLATE THIS ==" },
	{ IDS_WRITE_LAYOUT_MULTI_TEXTURE, "== TODO: TRANSLATE THIS ==" },
	{ IDS_WRITE_LAYOUT_MULTI_LAYER, "== TODO: TRANSLATE THIS ==" },
	...
	{ IDS_GENERATE_MESH_TOOLTIP_FBX, "Génère un fichier FBX contenant les objets 3D des couches sélectionnées, avec les références des matériaux. Requiert l'emplacement du dossier." },
	{ IDS_GENERATE_PNG_TOOLTIP_FBX, "Exporte toutes les textures et atlas des couches sélectionnées en tant que fichier(s) PNG. Requiert l'emplacement du dossier." },
	{ IDS_GENERATE_BOTH_TOOLTIP_FBX, "Génère à la fois un fichier FBX, contenant les objets 3D et les références des matériaux, et le(s) fichier(s) PNG correspondant(s) pour les couches et atlas sélectionnés. Requiert l'emplacement du dossier." },

	{ IDS_GENERATE_MESH_TOOLTIP_MAYA, "Génère les objets 3D des calques sélectionnés en créant des noeuds de scène et appliquant les matériaux appropriés." },
	{ IDS_GENERATE_PNG_TOOLTIP_MAYA, "Exporter toutes les textures et atlas des calques sélectionnés comme fichiers PNG dans un dossier localisé sous la source PSD." },
	{ IDS_GENERATE_BOTH_TOOLTIP_MAYA, "Génère tous les objets 3D en tant que noeuds de scènes et les textures en tant que PNG pour les calques sélectionnés, en plaçant les textures dans un dossier localisé sous la source PSD." },

	{ IDS_PSD_SELECTOR_TOOLTIP, "Localisation du fichier PSD sur le disque" },
	{ IDS_PSD_IMPORT_TOOLTIP, "Importer un fichier PDS à partir du disque" },
	{ IDS_PSD_RELOAD_TOOLTIP, "Recommencer l'importation du fichier PSD à partir du disque" },
	{ IDS_PSD_EXPORT_NAME_TOOLTIP, "== TODO: TRANSLATE THIS ==" },
	{ IDS_LAYER_LIST_TOOLTIP, "== TODO: TRANSLATE THIS ==" },
	{ IDS_SELECTED_PSD_SIZE_TOOLTIP, "Hauteur et largeur du fichier PSD" },
	{ IDS_GENERATION_ALGO_TOOLTIP_FBX, "<html><head/><body><p><span style=\" font-weight:600;\">Mode Delaunay </span>: Génère un objet 3D à densité ajustable en utilisant l’algorithme de triangulation Delauney. Requière un tracé fermé pour définir le périmètre de l’objet 3D, enregistré dans la liste des tracés Photoshop avec un nom correspondant à celui du calque. </p><p><span style=\" font-weight:600;\">Mode Vectoriel </span>: Génère un objet 3D à partir de courbes personnalisées dessinées par l’utilisateur. Requière un masque vectoriel Photoshop composé d’au moins un tracé à périmètre fermé et de tracés supplémentaires dessinés à travers le périmètre. Chaque croisement de tracé crée un sommet. </p><p><span style=\" font-weight:600;\">Mode Plan </span>: Génère un plan de travail rectangulaire recadré à la couche alpha. Ne require pas de configuration Photoshop supplémentaire. </p></body></html>" },
	{ IDS_GENERATION_ALGO_TOOLTIP_MAYA, "<html><head/><body><p><span style=\" font-size:8pt; font-weight:600;\">Mode Linéaire </span><span style=\" font-size:8pt;\">: Génère un objet 3D à densité ajustable en utilisant le modificateur de tessellation planaire NURBS dans Maya. Requière un tracé fermé pour définir le périmètre de l’objet 3D, enregistré dans la liste des tracés Photoshop avec un nom correspondant à celui du calque. </span></p><p><span style=\" font-size:8pt; font-weight:600;\">Mode Delaunay </span><span style=\" font-size:8pt;\">: Génère un objet 3D à densité ajustable en utilisant l’algorithme de triangulation Delauney. Requière un tracé fermé pour définir le périmètre de l’objet 3D, enregistré dans la liste des tracés Photoshop avec un nom correspondant à celui du calque. </span></p><p><span style=\" font-size:8pt; font-weight:600;\">Mode Vectoriel </span><span style=\" font-size:8pt;\">: Génère un objet 3D à partir de courbes personnalisées dessinées par l’utilisateur. Requière un masque vectoriel Photoshop composé d’au moins un tracé à périmètre fermé et de tracés supplémentaires dessinés à travers le périmètre. Chaque croisement de tracé crée un sommet. </span></p><p><span style=\" font-size:8pt; font-weight:600;\">Mode Plan </span><span style=\" font-size:8pt;\">: Génère un plan de travail rectangulaire recadré à la couche alpha. Ne require pas de configuration Photoshop supplémentaire. </span></p></body></html>" },
	{ IDS_BILLBOARD_ALPHA_THRESH_TOOLTIP, "<html><head/><body><p>L'algoritme plan englobe tous les pixels du calque avec la valeur alpha spécifiée ou une valeur inférieure. Des valeurs plus grandes produisent un plan plus petit et pourraient rogner les piels des bords. <br/><span style=\" font-weight:600;\">Plage [0 - 255]</span></p></body></html>" },
	{ IDS_LINEAR_PRECISION_TOOLTIP, "Nombre maximal de polygones dans l'objet 3D généré" },
	{ IDS_DELAUNAY_OUTER_DETAIL_TOOLTIP, "<html><head/><body><p align=\"justify\">Affecte le nombre de sommets le long du périmètre de l&quot;objet 3D. Des valeurs élevées produisent un maillage plus dense. <br/><span style=\" font-weight:600;\">Plage [1.0 - 100.0]</span></p></body></html>" },
	{ IDS_DELAUNAY_INNER_DETAIL_TOOLTIP, "<html><head/><body><p>Affecte le nombre de sommets à l'intérieur de l'objet 3D. Des valeurs plus élevées produisent un maillage plus dense. <br/><span style=\" font-weight:600;\">Plage [1.0 - 100.0]</span></p></body></html>" },
	{ IDS_DELAUNAY_INNER_FALLOFF_TOOLTIP, "<html><head/><body><p>Réduit le nombre de sommets vers le centre de l'objet 3D ; le périmètre n'est pas affecté. Des valeurs plus élevées réduisent la densité globale sans sacrifier les détails des contours. Définissez la valeur 0.0 pour la désactiver.<br/><span style=\" font-weight:600;\">Plage [0.0 - 100.0]</span></p></body></html>" },
	{ IDS_MERGE_VERTICES_TOOLTIP, "Si cette option est cochée, elle fusionne (soude) tous les sommets excédents le long du contour, réduisant ainsi la densité globale du maillage." },
	{ IDS_MERGE_VERTEX_DISTANCE_TOOLTIP, "<html><head/><body><p>La distance minimale entre les sommets dans l'espace UV le long du périmètre avant qu'ils ne soient fusionnés (soudés).<br/><span style=\" font-weight:600;\">Plage [0.001 - 0.2]</span></p></body></html>" },
	...
	{ IDS_TEXTURE_ATLAS_GROUP_TOOLTIP, "== TODO: TRANSLATE THIS ==" },
	...
	{ IDS_MESH_SCALE_TOOLTIP, "<html><head/><body><p>Affecte la taille de la scène à exporter. Des valeurs plus grandes augmentent la largeur et la hauteur de toutes les calques des objets 3D. <br/><span style=\" font-weight:600;\">Plage [0.1 - 1000.0]</span></p></body></html>" },
	...
	{ IDS_WRITE_MODE_TOOLTIP, "<html><head/><body><p><span style=\" font-weight:600;\">Binaire :</span> Le fichier FBX de sortie est stocké en format binaire. La taille du fichier est plus petite mais le contenu n'est pas lisible.</p><p><span style=\" font-weight:600;\">ASCII :</span> Le fichier FBX de sortie est stocké en format texte. Lisible par l'être humain mais la taille du fichier est plus importante.</p></body></html>" },
	{ IDS_WRITE_LAYOUT_TOOLTIP, "== TODO: TRANSLATE THIS ==" },
