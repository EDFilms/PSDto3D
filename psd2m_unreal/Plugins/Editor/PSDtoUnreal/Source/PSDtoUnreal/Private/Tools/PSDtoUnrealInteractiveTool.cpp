...
#include "PSDtoUnrealInteractiveTool.h"
...
#define LOCTEXT_NAMESPACE "UPSDtoUnrealInteractiveTool"

/*
 * ToolBuilder
 */

UInteractiveTool* UPSDtoUnrealInteractiveToolBuilder::BuildTool(const FToolBuilderState & SceneState) const
{
	UPSDtoUnrealInteractiveTool* NewTool = NewObject<UPSDtoUnrealInteractiveTool>(SceneState.ToolManager);
...
UPSDtoUnrealInteractiveToolProperties::UPSDtoUnrealInteractiveToolProperties()
{
...
void UPSDtoUnrealInteractiveTool::SetWorld(UWorld* World)
{
	check(World);
...
void UPSDtoUnrealInteractiveTool::Setup()
{
	...
	Properties = NewObject<UPSDtoUnrealInteractiveToolProperties>(this, "Measurement");
...
void UPSDtoUnrealInteractiveTool::OnUpdateModifierState(int ModifierID, bool bIsOn)
{
...
FInputRayHit UPSDtoUnrealInteractiveTool::CanBeginClickDragSequence(const FInputDeviceRay& PressPos)
{
...
void UPSDtoUnrealInteractiveTool::OnClickPress(const FInputDeviceRay& PressPos)
{
...
void UPSDtoUnrealInteractiveTool::OnClickDrag(const FInputDeviceRay& DragPos)
{
	UpdatePosition(DragPos.WorldRay);
}


FInputRayHit UPSDtoUnrealInteractiveTool::FindRayHit(const FRay& WorldRay, FVector& HitPos)
{
...
void UPSDtoUnrealInteractiveTool::UpdatePosition(const FRay& WorldRay)
{
...
void UPSDtoUnrealInteractiveTool::UpdateDistance()
{
	Properties->Distance = FVector::Distance(Properties->StartPoint, Properties->EndPoint);
}


void UPSDtoUnrealInteractiveTool::OnPropertyModified(UObject* PropertySet, FProperty* Property)
{
...
void UPSDtoUnrealInteractiveTool::Render(IToolsContextRenderAPI* RenderAPI)
{
